...
const topicsData = require('./aptitudeTopicsData');

const MONGODB_URI = process.env.MONGO_URI;

const seedAptitudeTopics = async () => {
  try {
    if (!MONGODB_URI) {
    ...
    console.log('Successfully seeded ' + count + ' aptitude topics.');
    process.exit(0);
  } catch (error) {
    console.error('Error seeding aptitude topics:', error);
    process.exit(1);
  }
};
